from tkinter import *
from tkinter.messagebox import showinfo

#stores the symbol for the present turn
y=""
#decides whose turn it is
x=2
#stores entries of players
player_1=[]
player_2=[]
f=0

#runs after the player has clicked on a button 
def define_sign(number):
    global x,y,f
    global player_1,player_2

    #generates all permutations
    from itertools import permutations
    set1 = permutations([1, 2, 3])
    set2 = permutations([4, 5, 6])
    set3 = permutations([7, 8, 9])
    set4 = permutations([1, 4, 7])
    set5 = permutations([1, 5, 9])
    set6 = permutations([3, 5, 7])
    set7 = permutations([2, 5, 8])
    set8 = permutations([3, 6, 9])

    for i in set1,set2,set3,set4,set5,set6,set7,set8:
        for j in list(i):
            plyr_1=all(elem in player_1 for elem in j)
            plyr_2 = all(elem in player_2 for elem in j)
            if plyr_1:
                print("Player 1 wins")
                showinfo("Congratulations","Player 1 has won!")
                clear()
                return
            elif plyr_2:
                print("Player 2 wins")
                showinfo("Congratulations","Player 2 has won!")
                clear()
                return
            else:
                pass

    if number==1:
        if x%2==0:
            if 1 in player_1 or 1 in player_2:  #checks for valid entry
                f=1
            if f==0:
                y='X'
                player_1.append(number)
                print(player_1)
        else:
            if 1 in player_1 or 1 in player_2:
                f=1
            if f==0:
                y='O'
                player_2.append(number)
                print(player_2)
        if f==0:
            b1.config(text=y,fg='#fc0303')
            x=x+1
        f=0
    elif number == 2:
        if x % 2 == 0:
            if 2 in player_1 or 2 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 2 in player_1 or 2 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f==0:
            b2.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    elif number == 3:
        if x % 2 == 0:
            if 3 in player_1 or 3 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 3 in player_1 or 3 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f == 0:
            b3.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    elif number == 4:
        if x % 2 == 0:
            if 4 in player_1 or 4 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 4 in player_1 or 4 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f == 0:
            b4.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    elif number == 5:
        if x % 2 == 0:
            if 5 in player_1 or 5 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 5 in player_1 or 5 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f == 0:
            b5.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    elif number == 6:
        if x % 2 == 0:
            if 6 in player_1 or 6 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 6 in player_1 or 6 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f == 0:
            b6.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    elif number == 7:
        if x % 2 == 0:
            if 7 in player_1 or 7 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 7 in player_1 or 7 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f == 0:
            b7.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    elif number == 8:
        if x % 2 == 0:
            if 8 in player_1 or 8 in player_2:
                f=1
            if f==0:
                y = 'X'
                player_1.append(number)
                print(player_1)
        else:
            if 8 in player_1 or 8 in player_2:
                f=1
            if f==0:
                y = 'O'
                player_2.append(number)
                print(player_2)
        if f == 0:
            b8.config(text=y,fg='#fc0303')
            x = x + 1
        f=0
    else:
        if x % 2 == 0:
            y = 'X'
            player_1.append(number)
            print(player_1)
        else:
            y = 'O'
            player_2.append(number)
            print(player_2)
        if f == 0:
            b9.config(text=y,fg='#fc0303')
            x = x + 1
        f=0


#resets boxes
def clear():
    b1.configure(text="")
    b2.configure(text="")
    b3.configure(text="")
    b4.configure(text="")
    b5.configure(text="")
    b6.configure(text="")
    b7.configure(text="")
    b8.configure(text="")
    b9.configure(text="")

    global x,y,f
    global player_1,player_2
    player_1=[]
    player_2=[]
    f=0
    x=2
    y=""

root=Tk()

l1=Label(root,text="Player 1 : X", font="times 15")
l1.grid(row=0,column=1)

l2=Label(root,text="Player 2 : O", font="times 15")
l2.grid(row=0,column=2)

b1=Button(root,width=20,height=10,command=lambda: define_sign(1))
b1.grid(row=1,column=1)

b2=Button(root,width=20,height=10,command=lambda: define_sign(2))
b2.grid(row=1,column=2)

b3=Button(root,width=20,height=10,command=lambda: define_sign(3))
b3.grid(row=1,column=3)

b4=Button(root,width=20,height=10,command=lambda: define_sign(4))
b4.grid(row=3,column=1)

b5=Button(root,width=20,height=10,command=lambda: define_sign(5))
b5.grid(row=3,column=2)

b6=Button(root,width=20,height=10,command=lambda: define_sign(6))
b6.grid(row=3,column=3)

b7=Button(root,width=20,height=10,command=lambda: define_sign(7))
b7.grid(row=5,column=1)

b8=Button(root,width=20,height=10,command=lambda: define_sign(8))
b8.grid(row=5,column=2)

b9=Button(root,width=20,height=10,command=lambda: define_sign(9))
b9.grid(row=5,column=3)

root.mainloop()

